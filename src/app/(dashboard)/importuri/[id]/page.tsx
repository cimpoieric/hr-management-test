...
function formatDisplayForKey(
  ...
  switch (key) {
    ...
    case "deploymentCountry": {
      ...
      const newName = resolved ? countries.find((c) => c.id === resolved)?.name ?? "—" : "—";
      return { oldLabel: dup.countryName ?? "—", newLabel: newName };
    }
    case "contractStartDate":
      return {
        oldLabel: formatDateDotRo(dup.salaryStartDate ?? dup.hiredAt),
        newLabel: formatDateDotRo(parseCimDateToUtcDate(edited.contractStartDate)),
      };
    case "grossSalary":
      return {
...
export default function ImportReviewPage() {
  ...
  const confirmUpdate = useCallback(async () => {
    ...
    setUpdating(true);
    try {
      const res = await fetch(`/api/employees/${data.duplicateEmployee.id}/update-from-import`, {
        method: "PUT",
      ...
      if (res.status === 409 && d.error === "NO_CHANGES") {
        ...
        toast.error(typeof d.message === "string" ? d.message : d.error ?? "Eroare la actualizare");
      }
    } catch {
  ...
  const importLocked = data.status === "COMPLETED_UPDATE" || data.status === "APPROVED";
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Revizuire import #{data.id}</h1>
        ...
        <div className="flex items-start gap-2 p-4 bg-emerald-50 border border-emerald-200 rounded-xl text-sm text-emerald-900">
          ...
          <div>
            ...
            <p className="mt-1 text-emerald-800">
              Acest import a fost folosit pentru actualizarea unui angajat existent.
            </p>
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                  ...
                  <div className="text-sm">
                    <p className="font-medium text-amber-900">Angajat existent detectat</p>
                    <p className="text-amber-800 mt-1 font-medium">Nume existent: {data.duplicateEmployee.name}</p>
                    <p className="text-amber-700 text-xs mt-1">CNP: {data.duplicateEmployee.cnp}</p>
                  </div>
                </div>
                <Link
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            <button
              ...
              title={hasDuplicate ? "CNP duplicat — folosește Actualizează angajat" : undefined}
        ...
        >
          <div className="bg-white rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto border border-gray-200">
            ...
            <div className="p-4 space-y-4 text-sm">
              <div>
                <p className="text-gray-600">
                  <span className="font-medium text-gray-800">Angajat:</span> {data.duplicateEmployee.name}
                </p>
                <p className="text-gray-600 mt-1">
                  <span className="font-medium text-gray-800">CNP:</span> {data.duplicateEmployee.cnp}
                </p>
              </div>

              <p className="font-medium text-gray-800">Selecționează câmpurile de actualizat:</p>
                    ...
                    >
                      <label className="flex gap-2 items-start cursor-pointer shrink-0 pt-0.5">
                        <input
                          ...
                          onChange={(e) =>
                            setModalChecked((prev) => ({ ...prev, [key]: e.target.checked }))
                          }
                          className="mt-1 rounded border-amber-400 text-amber-600 focus:ring-amber-500"
                        />
                        <span className="font-medium text-gray-800">{UPDATE_FIELD_LABELS[key]}</span>
                      </label>
                      <span className="text-gray-700 flex-1 min-w-0 break-words">
                        ...
                        <span className="text-gray-900 font-medium">{newLabel}</span>
                      </span>
                    </li>
                  );
                })}
            ...
            <div className="flex justify-end gap-2 p-4 border-t border-gray-100 bg-gray-50 rounded-b-xl">
              ...
              >
                <span className={cn("inline-flex shrink-0", !updating && "hidden")} aria-hidden={!updating}>
                  <Loader2 size={16} className="animate-spin" />
                </span>
