...
export default function AngajatDetailPage() {
  ...
  useEffect(() => {
    fetch(`/api/employees/${employeeId}`, { cache: "no-store" })
      ...
      .catch(() => router.push(ROUTES.employees))
    ...
    try {
      ...
      if (!res.ok) {
        setMessage({
          type: "err",
          text:
            typeof data.error === "string" ? data.error : "Eroare la salvare",
        });
        return;
      }
  ...
  return (
    <div className="bg-white rounded-xl border shadow-sm p-6 max-w-xl">
      ...
      <p className="text-xs text-gray-500 mb-4">
        Toate câmpurile sunt opționale. Tip plată gol = nesetat; sumă goală =
        fără sumă brută.
      </p>
    ...
    return (
      <div className="max-w-3xl space-y-3">
        <p className="text-sm text-gray-600">
          Editează date personale, bancare și contract. Salarizarea este în
          tab-ul <strong>Date salariale</strong>.
        </p>
        <EmployeeForm
          employeeId={employeeId}
          variant="infoOnly"
...
function HistoryTab({
  ...
  const [confirmDeleteItem, setConfirmDeleteItem] =
    useState<SalaryCalculationItem | null>(null);

  useEffect(() => {
    setLoading(true);
    fetch(`/api/employees/${employeeId}/salary-calculations?period=${period}`, {
      cache: "no-store",
    })
      .then((res) => res.json())
  ...
  async function handleDelete(itemId: number) {
    ...
    try {
      const res = await fetch(
        `/api/employees/${employeeId}/salary-calculations/${itemId}`,
        {
          method: "DELETE",
        },
      );
  ...
  return (
    <div className="bg-white rounded-xl border shadow-sm p-6">
      <div className="mb-4 flex items-center justify-between gap-3">
        <h3 className="text-base font-semibold text-gray-900">
          Istoric calcule salariu
        </h3>
        <select
          value={period}
          onChange={(e) =>
            setPeriod(e.target.value as "all" | "today" | "7d" | "30d")
          }
          ...
          {items.map((item) => (
            <div key={item.id} className="rounded-lg border p-3">
              ...
              <p className="text-xs text-gray-600 mt-1">
                Tip: {item.salaryType} · Bază: {item.salaryAmount}{" "}
                {item.salaryCurrency}
                {item.inputLabel &&
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full ${c.bg} ${c.text}`}
    >
      {c.label}
    </span>
  );
