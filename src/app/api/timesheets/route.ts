...
function parseOptionalInt(raw: string | null, fieldLabel: string): { value?: number; error?: string } {
  if (raw == null) return {};
  const s = raw.trim();
  if (!s || s.toLowerCase() === "null" || s.toLowerCase() === "undefined") return {};
  const n = Number(s);
  if (!Number.isFinite(n) || !Number.isInteger(n)) return { error: `${fieldLabel} trebuie să fie un număr valid` };
  return { value: n };
}
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (pageParsed.error) return NextResponse.json({ error: pageParsed.error }, { status: 400 });

    const pageSizeParsed = parseOptionalInt(searchParams.get("pageSize"), "Dimensiunea paginii");
    if (pageSizeParsed.error) return NextResponse.json({ error: pageSizeParsed.error }, { status: 400 });

    const employeeIdParsed = parseOptionalInt(searchParams.get("employeeId"), "employeeId");
    if (employeeIdParsed.error) return NextResponse.json({ error: employeeIdParsed.error }, { status: 400 });

    const weekParsed = parseOptionalInt(searchParams.get("weekNumber"), "Săptămâna");
    if (weekParsed.error) return NextResponse.json({ error: weekParsed.error }, { status: 400 });

    const yearRaw = searchParams.get("year");
    const yearParsedInt = parseOptionalInt(yearRaw, "Anul");
    if (yearParsedInt.error) return NextResponse.json({ error: yearParsedInt.error }, { status: 400 });
    ...
    const yearNormalized = yearParsedInt.value == null ? undefined : normalizeYear(yearParsedInt.value);
    if (yearNormalized?.error) return NextResponse.json({ error: yearNormalized.error }, { status: 400 });
    const year = yearNormalized?.value;

    // Zod: doar shape / tip (opțional) — nu mai impune valori fixe.
    const parsed = querySchema.safeParse({ page, pageSize, employeeId, year, weekNumber, status });
    if (!parsed.success) {
    ...
    return NextResponse.json({ error: "Eroare la listarea pontajelor" }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json({ error: parsed.error.issues[0]?.message ?? "Body invalid" }, { status: 400 });
    }
    ...
    if (existing) {
      return NextResponse.json(
        { error: "Există deja un pontaj pentru acest angajat în săptămâna/anul selectat" },
        { status: 400 }
      );
    }
    ...
    return NextResponse.json({ error: "Eroare la crearea pontajului" }, { status: 500 });
  }
}
