  ...
  try {
    ...
    for (const payslipId of payslipIds) {
      try {
        ...
        const netSalary =
          p.items.find((i) => i.type === "NET_SALARY")?.amount ?? 0;
        const travel =
          p.items.find((i) => i.type === "TRAVEL_ALLOWANCE")?.amount ?? 0;
        const holiday =
          p.items.find((i) => i.type === "HOLIDAY_MONEY")?.amount ?? 0;

        const r = await sendFluturasEmail({
          angajatEmail: toEmail,
          angajatNume:
            `${String(p.employee.lastName ?? "").trim()} ${String(p.employee.firstName ?? "").trim()}`.trim(),
