...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      console.error("[PAYSLIP_SEND_POST] generatePayslipPdf failed (continuă fără PDF atașat)", payslipId, e);
    }
    ...
    const result = await sendFluturasEmail({
      to: toEmail,
      angajatNume: `${String(payslip.employee.lastName ?? "").trim()} ${String(payslip.employee.firstName ?? "").trim()}`.trim(),
