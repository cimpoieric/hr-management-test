...
export function TimesheetForm({
  ...
  const { year: currentYear, week: currentWeek } = useMemo(() => currentIsoWeek(), []);
  ...
  const [weekNumber, setWeekNumber] = useState<number>(Math.min(52, Math.max(1, currentWeek)));
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          setEmployees([]);
          toast.error(e instanceof Error ? e.message : "Eroare la încărcarea angajaților");
        }
      }
    })();
    return () => {
  ...
  const filteredEmployees = useMemo(() => {
    ...
    return employees.filter((e) => {
      ...
      return name.includes(q) || String(e.id).includes(q) || String(e.position ?? "").toLowerCase().includes(q);
    });
            ...
            <div className="mt-5 flex-1 overflow-y-auto px-6 pb-4">
                  ...
                  <div className="mt-2 rounded-xl border bg-white shadow-lg overflow-hidden">
                    ...
                    <div className="max-h-56 overflow-auto">
                      ...
                      {filteredEmployees.length === 0 && (
                        <div className="px-3 py-4 text-sm text-gray-500">Niciun rezultat</div>
                      )}
                    </div>
                  </div>
                )}
              ...
              <div>
                ...
                <input
                  ...
                  value={weekNumber}
                  onChange={(e) => setWeekNumber(Math.min(52, Math.max(1, Number(e.target.value || "1"))))}
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-lg border px-3 py-2 text-sm"
                  value={year}
                  onChange={(e) => setYear(Number(e.target.value || String(currentYear)))}
                />
              </div>
